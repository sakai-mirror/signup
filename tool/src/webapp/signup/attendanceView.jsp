		...
		<sakai:view_content>
			...
			<h:form id="attendanceView">
				...
				<%--//TODO: attend.event.title below needs to resolve to the event title--%>
			 	<h3><h:outputText value="#{msgs.attend.view.title} /><h:outputText value="#{attend.event.title} styleClass="highlight"/></h3>
						...
						<t:column>
							<f:facet name="header" >
									...
									<h:outputText value="<input type="checkbox" id="selectAll" name="selectAll" /><label for="selectAll">" escape="false"/>
										<h:outputText value="#{msgs.attend.view.select.all}" escape="false" />
          ...
          <sakai:button_bar>
              <h:commandButton id="save" action="#{------}"   value="#{msgs.next_button}" />
              <h:commandButton id="cancel" action="#{------}" value="#{msgs.cancel_button}" /> 
          </sakai:button_bar>
